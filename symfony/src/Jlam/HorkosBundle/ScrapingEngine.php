...
abstract class ScrapingEngine implements Scrapper {
	...
	protected static $jurisdiction_shorthand;

	protected static $subjurisdiction;

	protected static $electionEngine;
	...
	protected static function setElectionEngine($class) {
		self::$electionEngine = $class;
	}


	protected static function setSubJurisdiction($subJur) {

		if(!empty($subJur)) {
			if(in_array($subJur, self::getSubJurisdictions())) {
				self::$subjurisdiction = $subJur;
			} else {
				self::addError("$subJur not in the list of valid subjurisdictions");
			}
		}
	}


	public static function getSubJurisdictions() {
		$kernelRootDir = self::$container->getParameter('kernel.root_dir');

		$jurisdiction = self::getJurisdictionShorthand();
		$engineClass = self::$electionEngine;
		$lang = $engineClass::langPathPartLookup(self::getLanguage());

		$filePath = $kernelRootDir . "/../../eshu/data/$jurisdiction/$lang/subJurs/subJurListUniq.txt";

		self::addLog("Opening sub jurisdiction list $filePath");

		$subJurArray = file($filePath);

		#Trim whitespace and remove array
		$subJurArray = array_filter(array_map('trim', $subJurArray));
	...
	public static function setJurisdictionShorthand($shorthand) {
		self::$jurisdiction_shorthand = $shorthand;
	}
	...
	public static function getJurisdictionShorthand() {
		return self::$jurisdiction_shorthand;
	}

...
maxRuns=100
completeStr='<td data-variable="Party">Ontario Liberal Party<span class="eresults-language-divider"> /</span><em>Parti lib'
dataFormat='json'
...
set +x
	...
	for lang in e ; do
        ...
        urlList=`curl 'https://www.elections.on.ca/en/election-results.html' | grep '/content/ngw/en/election-results/' | cut -d'"' -f 2`

		for relativeAddress in `echo $urlList`; do
            identifier=`echo $relativeAddress | cut -d'/' -f 6 | cut -d. -f 1`
            dataSource="https://www.elections.on.ca/$relativeAddress"
			ridingFile=$workDir/$identifier.$dataFormat
			readyFile=$readyDir/$identifier.$dataFormat
